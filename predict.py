    ...
    with tf.Graph().as_default():
       ...
       with tf.Session() as sess:
            ...
            # Run forward pass to calculate embeddings
                ...
                k=0     
	    #print predictions
                dir = os.listdir(args.image_files)
                for i in range(nrof_samples):
                    print("\npeople in image %s :" % (dir[i]))
...
def load_and_align_data(image_paths, image_size, margin, gpu_memory_fraction):
    ...
    dir = os.listdir(image_paths)
    nrof_samples = len(dir)
    img_list = [] 
    count_per_image = []
    for file in dir:
        img = misc.imread(os.path.join(os.path.expanduser(image_paths) , file))
        # arr = np.array(img)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--image_files', type=str, nargs='+', help='Path(s) of the image(s)', default='./data/casia-maxpy-clean/test/0155150')
    parser.add_argument('--model', type=str,
                        help='Could be either a directory containing the meta_file and ckpt_file or a model protobuf (.pb) file',
                        default='./models/facenet/20180408-102900')
    parser.add_argument('--classifier_filename',
                        help='Classifier model file name as a pickle (.pkl) file. ' +
                        'For training this is the output and for classification this is an input.',
                        default='./models/model_1.pkl')
